...
import (
	...
	"net/http"
    log "github.com/ochinchina/supervisord/logger"
)
...
func (lt *Logtail) getLog( logType string, w http.ResponseWriter, req *http.Request) {
    ...
    if proc == nil {
        ...
        var ok bool = false
        var compositeLogger *log.CompositeLogger = nil
        if logType == "stdout" {
            compositeLogger, ok = proc.StdoutLog.(*log.CompositeLogger )
        } else {
            compositeLogger, ok = proc.StderrLog.(*log.CompositeLogger )
        }
        if ok {
            ...
            chanLogger := log.NewChanLogger( ch )
